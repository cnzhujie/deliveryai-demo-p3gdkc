...
export default function App() {
  ...
  const handleToggleDark = () => {
    toggleDark()
    dispatch({ type: 'SET_MESSAGE', message: dark ? t('message.dark_mode_off') : t('message.dark_mode_on') })
  }
  ...
  return (
    <div className="min-h-screen bg-rice-100 paper-noise dark:bg-night-900">
      <TopBar
        table={state.table}
        view={state.view}
        ...
        <main className="mx-auto grid max-w-7xl gap-6 px-4 py-5 pb-28 lg:grid-cols-3 lg:px-6 lg:py-7 lg:pb-8">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-28">
              ...
              <div className="mt-4 rounded-2xl border border-amber-400/30 bg-amber-100/70 p-4 text-sm text-charcoal-700 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-rice-200/80">
      ...
      <div className="pointer-events-none fixed left-1/2 top-24 z-40 -translate-x-1/2 rounded-full bg-charcoal-900/90 px-4 py-2 text-xs font-semibold text-white shadow-float dark:bg-night-700/90">
        {state.lastMessage}
      </div>
    </div>
  )
